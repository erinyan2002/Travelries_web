...
- **Comment**: `post_comments` (`post_id`, `user_id`, `content`) — same visibility RLS as `post_likes` (author or a follower can read). `fetchComments`/`addComment`/`deleteComment`. Comments load lazily per post (only once its panel is expanded in the feed UI, `app/feed/page.tsx`'s `toggleComments`), not batched into `fetchFeed` — only `commentCount` is batched there, same N+1-avoidance pattern as like counts.
...
**Engagement notifications**: liking or commenting on a post notifies its author (`notifications` type `post_liked`/`post_commented`) — via `AFTER INSERT` triggers on `post_likes`/`post_comments` (`notify_post_like`/`notify_post_comment` in `0008_engagement_notifications.sql`), not client code, so it fires regardless of which call site does the insert. Both triggers (like `tag_user_in_post`) are `SECURITY DEFINER`, since `notifications` RLS only allows inserting your own rows and these need to insert one for the *other* user; both skip self-notifications (liking/commenting on your own post). `NotificationBell`'s `TYPE_ICON` map has entries for `post_liked`/`post_commented`/`tagged_in_post`, falling back to 🔔 for anything unmapped. For those three types, `data.from_user_id` (already stored by the trigger/RPC) is resolved to a name via `profilesByIds` and rendered as a `Link` to `/u/[id]` in place of the generic message — `share_viewed`/`collab_joined`/`collab_photo_added` predate this and keep their static `message` text.
